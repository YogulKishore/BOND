...
async def _post_response_tasks(
    ...
    if mediation_phase == "resolution":
        ...
        if beat_1 and not beat2_sent:
            ...
            if not beat_2:
                # LLM failed — leave phase as resolution, retry on next message
...
def create_session(couple_id: str, session_type: str, token: str):
    ...
    db = SessionLocal()
    try:
        # For shared sessions — require exactly 2 onboarded members
        if session_type == "shared":
            ...
            if len(members) < 2:
                return {"error": "shared_needs_partner", "message": "Your partner hasn't joined yet. Share your invite code with them first."}
            if not all(u.is_onboarded for u in members):
                return {"error": "shared_needs_onboarding", "message": "Both partners need to complete onboarding before starting a shared session."}
